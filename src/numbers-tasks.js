...
function getAverage(value1, value2) {
  ...
  const b = value2;
  const av = (a + b) / 2;
  return av >= Number.MAX_VALUE ? Number.MAX_VALUE : av;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modA = Math.abs(Math.sqrt(x1 ** 2 + y1 ** 2));
  const modB = Math.abs(Math.sqrt(x2 ** 2 + y2 ** 2));
  const cosAB = ab / (modA * modB);
...
function toNumber(value, def) {
  ...
  if (Number.isNaN(Num) === true) return def;
...
function isNumber(number) {
  let bool = true;
  const n = Number.isFinite(number);
  if (n === false) {
    bool = false;
  }
  return bool;
